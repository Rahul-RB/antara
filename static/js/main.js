(function () {
  ...
  function showSourcePanel(src, sourceSite) {
    ...
    const srcImgs = src.image_urls || [];
    const srcImg = srcImgs[0] || src.first_image_url;
    const srcSiteLabel = sourceSite === 'an' ? 'AN' : 'AS';
    srcPanel.innerHTML =
      '<div class="source-img">' +
        (srcImg ? '<img src="' + escHtml(srcImg) + '" alt="source profile">' : '<div class="no-image">No image</div>') +
      '</div>' +
  ...
  function showResults(result, sourceSite) {
    resultsArea.innerHTML = '';
    if (result && result.source) showSourcePanel(result.source, sourceSite);
    showMatches(result, sourceSite);
  }
